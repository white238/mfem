...
// *****************************************************************************
sedov::sedov(int argc, char *argv[])
{
   mpi = new MPI_Session(argc, argv);
   myid = mpi->WorldRank();
   ...
   L2FEC = new L2_FECollection(order_e, dim, BasisType::Positive);
   H1FEC = new H1_FECollection(order_v, dim);
   ...
   S = new BlockVector(true_offset, Device::GetMemoryType());
   S->UseDevice(true);
   ...
   DeltaCoefficient e_coeff(0.0, 0.0, 0.0, blast_energy);
   ...
   oper = new mfem::hydrodynamics::LagrangianHydroOperator(rho_coeff, S->Size(),
                                                           *H1FESpace, *L2FESpace,
